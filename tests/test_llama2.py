...
from nnfs.models.llama2 import Llama2, Llama2Config
from nnfs.modules import Llama2TransformerBlock
from nnfs.utils.model_io import build_model
from nnfs.utils.text_generation import generate


class TestLlama2Components(unittest.TestCase):
    def test_llama2_config_defaults(self):
        config = Llama2Config()
    ...
    def test_llama2_transformer_block(self):
        block = Llama2TransformerBlock(d_model=64, n_heads=8, n_kv_heads=2, d_ff=128, dropout=0.0)
...
class TestLlama2Model(unittest.TestCase):
    def setUp(self):
        self.config = Llama2Config(
            vocab_size=100,
            block_size=32,
            d_model=64,
            n_layers=2,
    ...
    def test_build_model_integration(self):
        model = build_model("configs/llama2_config.yaml")
        self.assertIsInstance(model, Llama2)
